...
*   **Student Group Management**: Admins can create and manage student groups for organizational purposes and bulk assignments. Group courses appear directly in "Mening Kurslarim" (no separate "Guruhlarim" page). Group course restrictions (schedule-based unlock, test gates) are enforced in the VideoLessonModal via the `lesson-lock-status` API. Supports **individual start dates** per student: when `useIndividualStartDate` is enabled in group course settings, each student's lesson unlock schedule starts from their `personalStartDate` (or `addedAt` if not set) instead of the group-wide `unlockStartDate`. Admins can set personal start dates when adding members or edit them individually in the members list.
...
*   **Flexible Test Timer System**: Tests support 4 timer modes: `none` (no timer), `global` (total test countdown in minutes), `per_question` (individual question timers in seconds with auto-advance), `sections` (question groups with individual/shared timers and optional sticky reading passages). Schema: `tests.timerMode`, `tests.timerValue`, `questions.timeLimit`, `questions.sectionId`, `test_sections` table (id, testId, title, orderIndex, timerType, timerValue, readingPassage). Instructor UI includes timer mode selector in test creation and sections management dialog with question-to-section assignment. Student UI has countdown displays, auto-submit on expiry, and section navigation with sticky reading text.
