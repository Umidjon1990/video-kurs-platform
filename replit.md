...
*   **Sequential Lesson Access**: Students must complete lessons in order - each lesson unlocks only after completing the previous one. Uses centralized `allSortedLessons` useMemo hook to ensure consistent ordering across sidebar and main content (considers module order + lesson order within modules). Progress tracked via `lesson_progress` table with `completed` boolean. Demo lessons are always accessible and don't block next lessons. Manual "Darsni tugalladim" button for lesson completion. Instructor/admin preview mode bypasses all locks.
