...
import { Calendar, Clock, AlertCircle, ArrowLeft, MessageCircle } from "lucide-react";
...
export default function InstructorSubscriptions() {
  ...
  const { data: allSubscriptions, isLoading: allLoading } = useQuery<any[]>({
    queryKey: ["/api/instructor/subscriptions"],
  });

  const { data: expiringSubscriptions, isLoading: expiringLoading } = useQuery<any[]>({
    queryKey: ["/api/instructor/subscriptions/expiring"],
  });

  const extendMutation = useMutation({
    mutationFn: async ({ id, days }: { id: string; days: number }) => {
      await apiRequest("PUT", `/api/instructor/subscriptions/${id}/extend`, {
        additionalDays: days,
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/instructor/subscriptions"] });
      queryClient.invalidateQueries({ queryKey: ["/api/instructor/subscriptions/expiring"] });
      toast({
